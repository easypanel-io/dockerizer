...
var ThreadWorker = class {
  ...
  initialize(options) {
    ...
    this.thread = new Worker(resolve(__dirname2, "./entry/worker.js"), options);
...
var ProcessWorker = class {
  ...
  initialize(options) {
    ...
    this.process = fork(resolve2(__dirname2, "./entry/process.js"), options);
...
var ThreadPool = class {
  ...
  runTask(task, options) {
    ...
    const taskInfo = new TaskInfo(task, transferList, filename, name, (err, result) => {
      ...
      if (this.shouldRecycleWorker(taskInfo)) {
        this._removeWorker(taskInfo.workerInfo).then(() => this._ensureMinimumWorkers()).then(() => this._ensureEnoughWorkersForTaskQueue()).then(() => resolve3(result)).catch(reject);
      } else {
        resolve3(result);
      }
